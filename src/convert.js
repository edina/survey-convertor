    ...
    fieldToJSON (id){
        ...
        switch (type) {
            ...
            case 'checkbox':
                ...
                $fieldId.find('input[name="'+id+'"]').each($.proxy(function(event){
                    var $img = $(this).closest(".form-inline").find("img");
                    if($img.length > 0){
                        checkboxes.push([]);
                        var n = checkboxes.length-1;
                        checkboxes[n].push($(this).val());
                        checkboxes[n].push(this.getFilenameFromURL($img.attr("src")));
                    }
                    else {
            ...
            case 'radio':
                ...
                $fieldId.find('input[name="'+id+'"]').each($.proxy(function(event){
                    ...
                    if($img.length > 0){
                        ...
                        radios[n].push(this.getFilenameFromURL($img.attr("src")));
                    }
                    else {
            ...
            case 'select':
                ...
                $fieldId.find('input[name="'+id+'"]').each($.proxy(function(event){
                    ...
                    if($img.length > 0) {
                        ...
                        options[n].push(this.getFilenameFromURL($img.attr("src")));
                    } else{
        ...
        $.each(this.form, function(key, value){
            ...
            switch (type) {
                ...
                case 'checkbox':
                    ...
                    $.each(value.checkboxes, $.proxy(function(k, v){
                        if(typeof(v) === "object"){
                            html.push('<label for="'+key+'-'+k+'">\n');
                            html.push('<div class="ui-grid-a grids">\n');
                            html.push('<div class="ui-block-a"><p>'+v[0]+'</p></div>\n');
                            html.push('<div class="ui-block-b"><img src="'+this.getFilenameFromURL(v[1])+'"></div>\n');
                            html.push('</label>');
                ...
                case 'radio':
                    html.push('<div class="fieldcontain" id="'+key+'" data-fieldtrip-type="'+type+'" '+persistent+' '+visibility+'>\n');
                    html.push('<fieldset>\n<legend>'+value.label+'</legend>\n');
                    $.each(value.radios, $.proxy(function(k, v){
                        if(typeof(v) === "object"){
                            html.push('<label for="'+key+'-'+k+'">\n');
                            html.push('<div class="ui-grid-a grids">\n');
                            html.push('<div class="ui-block-a"><p>'+v[0]+'</p></div>\n');
